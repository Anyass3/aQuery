...
const self = {
    __init__(query,arg){
        ...
        if(!this.new&&!$.is_html(query)){
            ...
            if(this.num>1||$.is_array(query)) this.many=true;
            if($.is_html(query))this.$$=query;
            else{
                this.$$=[...Array(this.num).keys()].reduce(
                    (arr)=>[...arr,document.createElement(query)],
                    []);
    ...
    //new init 
    $(query,num_for_new){
        if($.is_new(query,num_for_new))return this.$new
